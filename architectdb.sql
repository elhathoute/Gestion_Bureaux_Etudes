...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getInvoiceNotifications` ()   SELECT DISTINCT user_invoice.id_user, invoice.id AS id_invoice,invoice.id_client,invoice.F_number,invoice.date_creation
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_getNotifications` ()   SELECT DISTINCT user_devis.id_user, devis.id AS id_devis,devis.id_client,devis.number,devis.date_creation
